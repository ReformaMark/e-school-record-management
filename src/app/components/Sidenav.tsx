  ...
  return (
    <main className='hidden md:block fixed inset-0 w-[20%] bg-[#354649] pt-24 py-5 text-white shadow-lg'>
        ...
        <div className="px-2 py-5 space-y-2">
            ...
            <Link href={'/students'} className={`${pathname === '/students' ? "bg-background/50 text-white": "bg-transparent"} text-text flex items-center py-2 px-3 rounded-xl gap-x-3 text-sm font-medium  text-center`}>
                <PiStudent  className='size-5 w-10'/> 
                <h1 className='tracking-wider'>Enrollment</h1>
            </Link>
            <Link href={'/my-advisees'} className={`${pathname === '/my-advisees' ? "bg-background/50 text-white": "bg-transparent"}  text-text flex items-center py-2 px-3 rounded-xl gap-x-3 text-sm font-medium  text-center`}>
                <GiTeacher  className='size-5 w-10 '/>  
                
                <h1 className='tracking-wider '>Advisees</h1>
            </Link>
           
            <Link href={'/section'} className={`${pathname === '/section' ? "bg-background/50 text-white": "bg-transparent"} text-text  flex items-center py-2 px-3 rounded-xl gap-x-3 text-sm font-medium  text-center`}>
                <MdOutlineClass className='size-5 w-10 '/>
                <h1 className='tracking-wider '>Classes</h1>
            </Link>
            <Link href={'/assessments'} className={`${pathname === '/assessments' ? "bg-background/50 text-white": "bg-transparent"} text-text  flex items-center py-2 px-3 rounded-xl gap-x-3 text-sm font-medium  text-center`}>
