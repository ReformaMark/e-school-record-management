...
function SF9FrontTemplate({student}: SF9FrontTemplateProps) {
    ...
    useEffect(()=>{
        if(student.sectionDoc?.gradeLevel?.level === "Grade 11" || student.sectionDoc?.gradeLevel?.level === "Grade 12") {
            setIsSHS(true)
        }
    ...
    <div className='grid grid-cols-2 gap-x-10 p-10  text-black  w-full '>
        ...
        <div className="font-serif">
            <div className="grid grid-cols-12 py-10">
                ...
                {isSHS && (
                    <div className="flex justify-end items-start col-span-2">
                     <Image src={TanjayLogo} alt='TanjayLogo' width={100} height={100} className='size-20 object-contain'/>
                    </div>
                )}
               
                <div className={cn(isSHS ?"pl-0" :"pl-20" ,"col-span-12")}>
                    ...
                    <div className="grid grid-cols-12 gap-x-2 items-baseline gap-y-2 font-semibold">
                        <h1 className='col-span-12 flex gap-x-2'>Name:  <span className='font-normal text-center border-b-black border-b flex-1 px-2 inline-block capitalize'>{student.firstName} {student.middleName} {student.lastName} {student.extensionName}</span></h1>
                        {!isSHS && (
                        ...
                        <h1 className='col-span-8 flex gap-x-2'>Age:  <span className='font-normal border-b-black text-center border-b flex-1 px-2 inline-block'>{getStudentAge(student.birthDate)}</span></h1>
                        ...
                        <h1 className='col-span-5 flex gap-x-2'>Grade:  <span className='font-normal text-center  border-b-black border-b flex-1 px-2 inline-block'>{student.sectionDoc?.gradeLevel?.level}</span></h1>
                        ...
                        <h1 className='col-span-12 flex gap-x-2'>School Year: <span className='font-normal text-center border-b-black border-b flex-1 px-2 inline-block'>{student.cLass.schoolYear.sy}</span></h1>
                        {isSHS && (
